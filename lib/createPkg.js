...
module.exports = function(npmName, makePkgArgv, options, cb) {
  ...
  return npm2arch(npmName, options, function(err, pkgbuild) {
    ...
    return fs.mkdir(tmpDir, '0755', function(err) {
      ...
      return fs.writeFile(path.join(tmpDir, 'PKGBUILD'), pkgbuild, function(
        err
      ) {
        if (err) {
          return cb2(err);
        ...
        return child.on('exit', function(code) {
          ...
          if (code !== 0) {
            return cb2(`Bad status code returned from \`${makepkg}\`: ${code}`);
          }
          ...
          return fs.readdir(tmpDir, function(err, files) {
            ...
            const pkgFile = files.filter(
              file => file.indexOf('nodejs-') === 0
            )[0];
            const newPkgFile = path.join(process.cwd(), path.basename(pkgFile));
            if (fs.existsSync(newPkgFile)) {
              fs.unlinkSync(newPkgFile);
            }
            return fs.move(path.join(tmpDir, pkgFile), newPkgFile, function(
              err
            ) {
              if (err) {
                return cb2(err);
